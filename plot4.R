...
#plot 4
#set mfrow
par(mfrow = c(2,2), mar = c(4.5,4.1,0.5,2.1))
#1
...
#2
plot(as.numeric(data2$Voltage), type = "n", xaxt = "n", xlab = "datetime", ylab = "Voltage")
lines(as.numeric(data2$Voltage), col = "black")
axis(side = 1, at = c(1,nrow(data2)/2,nrow(data2)), labels = c(as.character(wday(d1, label = TRUE)), as.character(wday(d2, label = TRUE)), as.character(wday(d2+1, label = TRUE))))
...
#4
plot(as.numeric(data2$Global_reactive_power), type = "n", xaxt = "n", xlab = "datetime", ylab = "Global_reactive_power")
lines(as.numeric(data2$Global_reactive_power), col = "black")
axis(side = 1, at = c(1,nrow(data2)/2,nrow(data2)), labels = c(as.character(wday(d1, label = TRUE)), as.character(wday(d2, label = TRUE)), as.character(wday(d2+1, label = TRUE))))
dev.off()
